...
function institutional_impact!(S;M=100, inst=1)
    ...
    for q in S
        ...
        push!(s.institutional_impacts,(target=collect(t),id_total=t[argmax(total)],id_resource=t[argmax(resource)],id_gini=t[argmin(gini)],total=total,resource=resource,gini=gini,I=I,y=y,U=U, institution=string(typeof(S[1].institution[1]))*" "*string((hasfield(typeof(S[1].institution[1]),:target) ? S[1].institution[1].target : nothing))))
    end 
end
