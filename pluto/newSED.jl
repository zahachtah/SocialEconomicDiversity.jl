...
# ╔═╡ 0723229c-a51c-11ef-0cdd-b35be66e2551
using ModelingToolkit, DifferentialEquations, CairoMakie, BenchmarkTools

# ╔═╡ 1235837a-ccc4-42cc-aa2d-3cec33a77fe9
md"
* Get r and K from spatial models
* implement incentive based permits, markets and tax/subsidies
* Find scenarios that show when different policy instruments work best
"

# ╔═╡ 6316216c-6e48-48d7-bfa2-911859376030
...
# ╔═╡ b160596a-132c-4404-9e76-bd69a472f24e
function stage_limiter!(x, integrator, p, t)
	...
	# Optional variable in positive range
	x[p.N+2]=max(0.0,x[p.N+2])
end

# ╔═╡ b67b94e5-4871-48dd-8dc4-a0a9e45e0dce
N=200

# ╔═╡ c6fbf006-4375-4679-85da-16e5b455c401
begin
	P=zeros(N)
	P[1:Int64(N/2)].=1.0
	P
...
# ╔═╡ 23aaf0a1-6a4e-4e47-9bc0-c199ad359578
function γ(x,p,t)
	return p.w̃.+P.*0.5
end

# ╔═╡ 0451c102-e068-4ff5-a4af-d25218fd0b12
function ϕ(x,p,t)
	return 0.0
end

# ╔═╡ 416847aa-f41d-47d9-a569-65da2932f7d6
function μ(x,p,t)
	return p.ū
end

# ╔═╡ b59f20c7-e588-4e89-9ffd-98396970102d
p=(N=N,α=0.1,w̃=range(0.1,stop=0.9,length=N),ū=fill(1.0/N,N), γ=γ,ϕ=ϕ, μ=μ)

# ╔═╡ ef10f275-b2e8-465e-b360-02bc81a24623
begin
	u0=zeros(N)
	y0=1.0
	ϕ0=0.0
...
# ╔═╡ f5b17fb6-1809-4379-bada-940673fc08fd
 prob = ODEProblem(dxdt,[u0;y0;ϕ0],tend,p);

# ╔═╡ 14257f83-270a-4226-9445-085db787b8d4
	sol=solve(prob,SSPRK432(;stage_limiter!),callback= TerminateSteadyState(1e-6,1e-4) )

# ╔═╡ f9335302-b951-4fbe-be57-ad5133f1e8dd

@btime solve(prob,SSPRK432(;stage_limiter!),callback= TerminateSteadyState(1e-6,1e-4) )

# ╔═╡ 27a67ce0-2dfb-429e-b455-81e388b2faf9
function Φ(y,p)
	x=zeros(p.N+1)
	x[end]=y
	γ=p.γ(x,p,0.0)
...
# ╔═╡ 88d49d4e-c249-49a8-a0dd-9e71599a277c
scatter(p.w̃,sol.u[end][1:p.N])

# ╔═╡ da23adc9-84e6-4eb0-97a5-2db225c35028
y=range(0.0,stop=1.0,length=100)

# ╔═╡ f363aec7-401d-4a0d-9421-db6ae4d5bd39
scatter(y,Φ.(y,Ref(p)), color=p.w̃)

# ╔═╡ 75e56dac-a5dd-4464-b075-2e5ecc1d2de3
scatter(γ([],p,0),p.:w̃)

# ╔═╡ da7d2747-c2a7-4a7d-944e-20c9f88d051e
scatter([u[p.N+1] for u in sol.u],[sum(u[1:p.N]) for u in sol.u])
...
manifest_format = "2.0"
project_hash = "cf6789b3750f46e446593caf3c9f5aa48a4c801c"
