...
function ShowGraphicalElements()
	...
	s=scenario(color=:steelblue, q=SED(min=1.0,max=2.0,normalize=true),institution=[Dynamic_permit_allocation(:w̃,false,:inorder,0.2,:value,dynamic_permits)]); dist!(s); sim!(s)
	phaseplot!(a11,s, show_trajectory=true, show_realized=true, indicate_incentives=:w)
	incomes!(a12,s, indexed=true)
	SEDplot!(a21,s,:w,:w̃)
	individual_u!(a22,s)
	xlims!(a22,(0,20))
